...
for numtx=1:4      %1:4 Transmitters (1-4)
    pause(1)
    for imcs=8  %1:m % MCS to evaluate
        for k=1:numiter % Number of packets
            mcs_sel = mcs_all(numtx,imcs);
...
%% Variables inside Function
            ...
            if hardware
                ...
                LTF_thresh              = 0.8;          % Higher threshold for Hardware
            else
            ...
            NumReceiveAntennas          = cfgHT.NumTransmitAntennas;  % Number of Rx antennas
            ...
            PacketErrors=0; % Initialize as no Packet error
            ...
            fprintf('\nTransmitters %d at MCS %d, %s : %d Mbps, Hardware %d\n',numtx, mcs_sel, MCSinfo.ModText,MCSinfo.Datarate/1e6 ,hardware);
%% SETUP WARPLAB
            if(USE_WARPLAB_TXRX)
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%                 rx_vec_air = rx_vec_air;
            ...
            % LTS Correlation
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            pktStartIdx = packetDetect(rx,cfgHT.ChannelBandwidth);
            if isempty(pktStartIdx) % If empty no L-STF detected; packet error
                NoLTS=1; % No LTS
                PacketErrors=1; % Bad Packet
            end
...
%             rxPSDU = wlanHTDataRecover(htdata,chanEst,nVar,cfgHT);
            ...
            if ConstPlot
                [ cf ] = plot_SymConst(numtx , cf, rx_syms, MCSinfo);
            end
            
        end % Number of iteratinon
    end % mcs
end % Loop num TX