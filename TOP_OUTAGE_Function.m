...
% Top Function file for Stream Outage Evaluation
...
rxgains_all= ...
    ...
    9 8 9 10];   %4ss   10 9 9 10
failedit=[];

% EVALUATION KNOBS
mcstoeval=[1 2 3 4 5 6 7 8]; %MCS to eval per transmission (1-8) see comment above
 mcstoeval=[4];
outagetoeval=[0]; % CFO to eval in Hz to be applied in Time before transmission
numtxarr=[2 3 4];
%% Run Script
for numtxarrInd=1:length(numtxarr)
    numtx=numtxarr(numtxarrInd); %Transmitters 4 
for outageindex=1:length(outagetoeval) % KNOB - x-axis save per file
    
    outage_sel = outagetoeval(outageindex);
    % Filename to save data
    filename=sprintf('/media/rrng/HD1TB/Outage/Outage_OUT%d_Tx%d_Iter%d_feb20_DH1049.mat',outage_sel,numtx,numiter);
    %     filename=sprintf('/home/rrng/Dropbox/ChameleonExperiments/Outage//Data/Outage_OUT%d_Tx%d_Iter%d.mat',outage_sel,numtx,numiter);
    ...
    for imcs=1:length(mcstoeval)  %MCS vector to evaluate (1 to 8)
        for k=1:numiter % Number of packets
            try
                ...
                MAX_TX_LEN                  = 2^14;        % Maximum number of samples to use for this experiment
                ...
                fprintf('\nOutages: %d, Transmitters %d at MCS %d, %s : %d Mbps, Hardware %d Iteration: %d\n',outage_sel, numtx, mcs_sel, MCSinfo.ModText,MCSinfo.Datarate/1e6 ,hardware, k);
                %% SETUP WARPLAB
                if(USE_WARPLAB_TXRX)
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%                     wl_interfaceCmd(node_tx4,RFA, 'channel',2.4, chan24);
                ...
                % Outage
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                % Remove streams NOT transmitting ( No Backlogged )
                
                elim=sort(randperm(numtx,outage_sel)); % which streams to Not Transmit
                tx_vec_air(:,elim)=0;
