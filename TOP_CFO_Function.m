...
% Top Function file for phase synchronization
...
hardware=1; % Using Hardware if 0 = Simulation
...
mcstoeval=[1]; %MCS to eval per transmission (1-8) see comment above
packetlength=[500];
cfotoeval=[0 400 800 1200 1500 2000]; % CFO to eval in Hz to be applied in Time before transmission
% cfotoeval=[0]; % CFO to eval in Hz to be applied in Time before transmission

%% Run Script
for pktLengthIdx=1:length(packetlength)
    pktlenPSDU=packetlength(pktLengthIdx);
    for cfoindex=1:length(cfotoeval) % KNOB - x-axis save per file
        numtx=2; %Transmitters 2 (NO loop)
         cfo_sel = cfotoeval(cfoindex);
        % Filename to save data
        filename=sprintf('/media/rrng/HD1TB/Phase/Phase_CFO%d_PL_%d_CFO_Tx%d_Iter%d_feb20_DH1049.mat',pktlenPSDU,cfo_sel,numtx,numiter);
        for imcs=1:length(mcstoeval)  %MCS vector to evaluate (1 to 8)
            for k=1:numiter % Number of packets
                ...
                cfgHT.PSDULength            = pktlenPSDU;  % PSDU length in bytes%Max possible 2620 bytes
                cfgHT.MCS                   = mcs_sel;    %MCS
                NumReceiveAntennas          = 2;%cfgHT.NumTransmitAntennas;  % Number of Rx antennas
                MCSinfo                     = util_getMCSinfo(mcs_sel); % ALl info of MCS

                % Plotting and RX Stats
                cf                          = 99; % Figure counter
                ConstPlot                   = 1; % Show Constellation Plot
                ...
                fprintf('\nPl: %d, CFO: %d, Transmitters %d at MCS %d, %s : %d Mbps, Hardware %d Iteration: %d\n',pktlenPSDU,cfo_sel, numtx, mcs_sel, MCSinfo.ModText,MCSinfo.Datarate/1e6 ,hardware, k);
                %% SETUP WARPLAB
                if(USE_WARPLAB_TXRX)
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
size(tx_syms)
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                % SHIFT in Frequency ( CFO)
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                 tx_vec_air(:,2) = util_AddCFO(tx_vec_air(:,2),cfo_sel);
