...
% Top Function file for Time Synchronization
% This file is used by all experiment TOP files
% Vaiables are modified depending on the experiment

% Warning: Node 1 with Serial # 210 has a delay step of 6.250 ns and a maximum delay
% of 193.750 ns.
% Requested output delay of 180.000 ns actually set to 175.000 ns.
% > In wl_trigger_manager_proc/procCmd (line 311)
%   In wl_node/wl_triggerManagerCmd (line 425)
%   In TOP_Function_Chameleon (line 183)
...
rxgains_all= ...
    [ 11 12 12 11;... %1ss
    11 11 12 12;... %2ss
    9 9 11 9;... %3ss 10 9 9 9
    10 9 9 10];   %4ss   10 9 9 10
...
%   mcstoeval=[8]; %MCS to eval per transmission (1-8) see comment above
 ...
 delaytoeval=[0 100 200 300 400 500 600 700 800 900 1000]; % Trigger delay in ns
%      delaytoeval=[0]; % Trigger delay in ns

%% Run Script
for delayindex=1:length(delaytoeval)
    numtx=2; %Transmitters 2 (NO loop)
    delay_sel = delaytoeval(delayindex);
    numzeros=delay_sel/50; % Each IQ sample (0) represents 50 ns  - because each ofdm symbol is 3.2us and has 64 IQ samples
    % Filename to save data
    filename=sprintf('/media/rrng/HD1TB/Timing/Timing_Delay%d_Tx%d_Iter%d_feb28_DH1049.mat',delay_sel,numtx,numiter);
    %     filename=sprintf('/media/rrng/HD1TB/Timing/Timing_Tx%d_Iter%d.mat',numtx,numiter);
    %     filename=sprintf('/home/rrng/Dropbox/ChameleonExperiments/Timing/Data/Timing_Delay%d_Tx%d_Iter%d.mat',delay_sel,numtx,numiter);
    
    for imcs=1:length(mcstoeval)  %MCS vector to evaluate (1 to 8)
        for k=1:numiter % Number of packets
            try
                mcs_sel = mcs_all(numtx,mcstoeval(imcs)); % selected MCS for run
                ...
                MAX_TX_LEN                  = 2^15;        % Maximum number of samples to use for this experiment
                chan24                      = 14;           % Channel at 2.4 GHz
                USE_EXTERNAL_TRIGGER        = 1; % Use external trigger between Tx nodes
                %             trigger_delay               = [delay_sel delay_sel delay_sel]; % Delay per slave 1,2 or 3
                trigger_delay               = [0 0 0 0]; % Delay per slave 1,2 or 3
                ...
                fprintf('\nDelay: %d, Transmitters %d at MCS %d, %s : %d Mbps, Hardware %d Iteration: %d\n',delay_sel, numtx, mcs_sel, MCSinfo.ModText,MCSinfo.Datarate/1e6 ,hardware, k);
                %% SETUP WARPLAB
                if(USE_WARPLAB_TXRX)
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                % SHIFT in TIME for Trigger
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                if numzeros > 0
                    tx_temp= [tx; [zeros(numzeros,2)]];
                    tx_temp(:,2)=[zeros(numzeros,1);tx_temp(1:end-numzeros,2)]; %Added zeros
                    tx=tx_temp;
                end
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                % INTERPOLATION
